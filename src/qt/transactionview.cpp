    ...
    transactionView(0)
{
    ...
    QAction *viewOnGigachain = new QAction(tr("Show transaction on Gigachain"), this);
    ...
    contextMenu->addSeparator();
    contextMenu->addAction(viewOnGigachain);
    ...
    connect(viewOnGigachain, SIGNAL(triggered()), this, SLOT(viewOnGigachain()));
}
...
void TransactionView::viewOnGigachain()
{
    QModelIndexList selection = transactionView->selectionModel()->selectedRows();
    if(!selection.isEmpty())
    {
        QString format("http://gigachain.info/tx/");
