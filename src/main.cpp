...
#if defined(NDEBUG)
# error "Gigacoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Gigacoin Signed Message:\n";
...
bool CheckTransaction(const CTransaction& tx, CValidationState &state)
{
    ...
    if (tx.IsCoinBase())
    {
        if (tx.vin[0].scriptSig.size() < 2 || tx.vin[0].scriptSig.size() > 130)
...
{
    //1M coins total. 10000 premine, 40 per block after, etc etc
    int64_t reward = 50 * COIN;
    int64_t sub=0;
    if(nHeight==2)
    {
        sub=1000*reward; //premine of 10000 coins, 0.5% of cap
    }
...
static const int64_t nTargetTimespan = 60 ; // Gigacoin: every 1 minute
static const int64_t nTargetSpacing = 60; // Gigacoin: 1 minute
...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
    ...
    // Gigacoin: This fixes an issue where a 51% attack can change difficulty at will.
    ...
    */
    int64_t adjust=0;
    if(pindexLast->nHeight > (HOUR*DAY*7) && pindexLast->nHeight < (108973)) //don't activate til rewards drop
    {
...
void ThreadScriptCheck() {
    RenameThread("gigacoin-scriptch");
...
bool ConnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // BIP16 was always active in Gigacoin
...
{
    // Gigacoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
